    ...
    for (i = dpiv; i < ncols; ++i) {
        ...
        if (dts[DENSE] == 0) {
            ...
            for (; j < len; j += 16) {
                ...
                redv = vld1q_u32((cf32_t *)(cfs)+j);
                resv = vmlal_u32(drv, vget_low_u32(redv), mulv);
                vst1q_u64(drs+j, resv);
                drv  = vld1q_u64(drs+j+2);
                resv = vmlal_u32(drv, vget_high_u32(redv), mulv);
                vst1q_u64(drs+j+2, resv);
                drv  = vld1q_u64(drs+j+4);
                redv = vld1q_u32((cf32_t *)(cfs)+j+4);
                resv = vmlal_u32(drv, vget_low_u32(redv), mulv);
                vst1q_u64(drs+j+4, resv);
                drv  = vld1q_u64(drs+j+6);
                resv = vmlal_u32(drv, vget_high_u32(redv), mulv);
                vst1q_u64(drs+j+6, resv);
                drv  = vld1q_u64(drs+j+8);
                redv = vld1q_u32((cf32_t *)(cfs)+j+8);
                resv = vmlal_u32(drv, vget_low_u32(redv), mulv);
                vst1q_u64(drs+j+8, resv);
                drv  = vld1q_u64(drs+j+10);
                resv = vmlal_u32(drv, vget_high_u32(redv), mulv);
                vst1q_u64(drs+j+10, resv);
                drv  = vld1q_u64(drs+j+12);
                redv = vld1q_u32((cf32_t *)(cfs)+j+12);
                resv = vmlal_u32(drv, vget_low_u32(redv), mulv);
                vst1q_u64(drs+j+12, resv);
                drv  = vld1q_u64(drs+j+14);
                resv = vmlal_u32(drv, vget_high_u32(redv), mulv);
                vst1q_u64(drs+j+14, resv);
            }
        }
#else
        ...
        )
{
    ...
    for (i = dpiv; i < ncols; ++i) {
        ...
        if (dts[DENSE] == 0) {
            ...
            for (; j < len; j += 16) {
                redv   = _mm512_setr_epi64(
                        ...
                        cfs[j+6],
                        cfs[j+7]);
                drv  = _mm512_loadu_si512((__m512i*)(drs+j));
                /* first four mult-adds -- lower */
                prodv = _mm512_mul_epu32(mulv, redv);
                resv  = _mm512_sub_epi64(drv, prodv);
                cmpv  = _mm512_cmpgt_epi64_mask(zerov, resv);
                rresv = _mm512_mask_add_epi64(resv, cmpv, resv, mod2v);
                _mm512_storeu_epi64(drs+j, rresv);
                /* second four mult-adds -- higher */
                drv  = _mm512_loadu_si512((__m512i*)(drs+j+8));
                redv   = _mm512_setr_epi64(
                        cfs[j+8],
                        cfs[j+9],
                        cfs[j+10],
                ...
                cmpv  = _mm512_cmpgt_epi64_mask(zerov, resv);
                rresv = _mm512_mask_add_epi64(resv, cmpv, resv, mod2v);
                _mm512_storeu_epi64(drs+j+8, rresv);
            }
        }
#elif defined HAVE_AVX2
        const len_t len = dts[LENGTH];
        ...
        if (dts[DENSE] == 0) {
            ...
            for (; j < len; j += 8) {
                drv  = _mm256_loadu_si256((__m256i*)(drs+j));
                redv = _mm256_setr_epi64x(
                        cfs[j],
                        cfs[j+1],
                        cfs[j+2],
                        cfs[j+3]);
                ...
                cmpv  = _mm256_cmpgt_epi64(zerov, resv);
                rresv = _mm256_add_epi64(resv, _mm256_and_si256(cmpv, mod2v));
                _mm256_storeu_si256((__m256i*)(drs+j), rresv);
                /* second four mult-adds -- higher */
                drv  = _mm256_loadu_si256((__m256i*)(drs+j+4));
                redv  = _mm256_setr_epi64x(
                        cfs[j+4],
                        cfs[j+5],
                        cfs[j+6],
                ...
                cmpv  = _mm256_cmpgt_epi64(zerov, resv);
                rresv = _mm256_add_epi64(resv, _mm256_and_si256(cmpv, mod2v));
                _mm256_storeu_si256((__m256i*)(drs+j+4), rresv);
            }
        }
#elif defined __aarch64__
        const len_t len       = dts[LENGTH];
        ...
        if (dts[DENSE] == 0) {
            ...
            for (; j < len; j += UNROLL) {
                ...
                drs[j+3] -=  mul * cfs[j+3];
                drs[j]   +=  (drs[j] >> 63) & mod2;
                drs[j+1] +=  (drs[j+1] >> 63) & mod2;
                drs[j+2] +=  (drs[j+2] >> 63) & mod2;
                drs[j+3] +=  (drs[j+3] >> 63) & mod2;
            }
        }
#endif
        dr[i] = 0;

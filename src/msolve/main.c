...
static inline void display_help(char *str){
  ...
  display_option_help('b', "byteencoding", "BENC", "Define the byte encoding of the input for field extensions\n");
  display_option_help_noopt("0 - Interpret coefficients modulo the subfield characteristic.\n");
  display_option_help_noopt("1 - Interpret coefficients as byte representations in the field extension. (default)\n");
...
static void getoptions(
  ...
  struct option long_options[] = {
    {"byteencoding", required_argument, NULL, 'b'},
  ...
  while(1) {
    ...
    case 'C':
      *colon = 1;
      break;
    case 'b':
      *byte_encoding = strtol(optarg, NULL, 10);
      if (*byte_encoding > 1 || *byte_encoding < 0) {
        *byte_encoding = 1;
      }
      break;
    case 'r':
...
int main(int argc, char **argv){
    ...
    getoptions(argc, argv, &initial_hts, &nr_threads, &max_pairs,
               ...
               &precision, &refine, &isolate, &generate_pbm, &byte_encoding,

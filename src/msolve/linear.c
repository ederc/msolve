...
static inline void check_and_set_linear_poly_gf_16_256(nvars_t *nlins_ptr,
                                                nvars_t *linvars,
                                                uint32_t **lineqs_ptr,
                                                ht_t *bht, int32_t *bexp_lm,
                                                bs_t *bs) {
  ...
  *nlins_ptr = nlins;
  ...
  for (int i = 0; i < bht->nv; i++) {
    if (linvars[i] != 0) {
      ...
      if (len == bht->nv + 1) {
        for (long j = 0; j < len; j++) {
          uint32_t coef = bs->cf2_ext[bs->hm[bi][COEFFS]][j];
          lineqs[cnt * (bht->nv + 1) + j] = coef;
        }
      } else {
        hm_t *dt = bs->hm[bi] + OFFSET;
...
static inline void check_and_set_linear_poly(nvars_t *nlins_ptr, nvars_t *linvars,
  ...
  if (bs->cf_8 != NULL) {
    ...
    check_and_set_linear_poly_16(nlins_ptr, linvars, lineqs_ptr, bht, bexp_lm,
                                 bs);
  } else if (bs->cf_32 != NULL) {
    check_and_set_linear_poly_32(nlins_ptr, linvars, lineqs_ptr, bht, bexp_lm,
                                 bs);
  } else if (bs->cf2_ext != NULL) {
    check_and_set_linear_poly_gf_16_256(nlins_ptr, linvars, lineqs_ptr, bht, bexp_lm,
                                 bs);
  }
}
